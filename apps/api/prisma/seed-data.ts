...
// Reference data and builders shared by the dev seed (seed.ts) and the large seed (seed-large.ts).

export const DESIGNATIONS = ['Apprentice Technician', 'Technician', 'Senior Technician', 'Lead Technician']

// Days since hire by designation, so apprentices are recent hires and leads have been around for years.
export const TENURE_DAYS: Record<string, [min: number, max: number]> = {
...
export type CitySpec = [houseNumbers: [min: number, max: number], postalPrefixes: string, streets: string[]]
export type RegionSpec = { areaCodes: string[]; cities: Record<string, CitySpec> }

// Province › region › city. City names are unique across the whole geography.
export const GEOGRAPHY: Record<string, { name: string; regions: Record<string, RegionSpec> }> = {
  BC: {
...
export const POSTAL_LETTERS = 'ABCEGHJKLMNPRSTVWXYZ'.split('')

export type JobTemplate = [title: string, description: string]

// Category › specialty › job templates. Specialty names are globally unique.
export const TAXONOMY: Record<string, Record<string, JobTemplate[]>> = {
  HVAC: {
    Furnaces: [
...
export const ROSTER: [city: string, load: number][] = [
...
export const IN_REGION_PROBABILITY = 0.85

// Specialties per technician, drawn from one primary category plus sometimes a second one.
export const SPECIALTIES_PER_TECHNICIAN: [min: number, max: number] = [2, 5]
export const SECOND_CATEGORY_PROBABILITY = 0.5

// Priority mix of generated jobs: mostly medium, a few urgent.
export const PRIORITY_WEIGHTS: { weight: number; value: Priority }[] = [
  { weight: 3, value: 'LOW' },
...
export type SeedSkill = { id: string; name: string; categoryId: string }
export type SeedRegion = { id: string; name: string; provinceCode: string }
export type SeedCity = { id: string; name: string; regionId: string }

export const jobTemplates = new Map<string, JobTemplate[]>()
export const citySpecs = new Map<string, CitySpec>()
export const areaCodes = new Map<string, string[]>()
...
export function findByName<T extends { name: string }>(items: T[], name: string): T {
