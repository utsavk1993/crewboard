...
export interface AssignedJobsCardProps {
  ...
  retrying: boolean
  onRetry: () => void
  /** Job whose assignment is changing, if any. */
...
function JobItem({ job, pendingJobId, onUnassign }: JobItemProps) {
  return (
    <li className="flex flex-col gap-1 px-5 py-3.5">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        <p className="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
        ...
        <Button
          variant="ghost"
          size="sm"
          // One change at a time, so the list and counts settle before the next one.
          disabled={pendingJobId !== null}
          onClick={() => onUnassign(job)}
          className="-mr-2.5 shrink-0 text-destructive hover:bg-destructive/10 hover:text-destructive dark:hover:bg-destructive/10"
        >
...
function Body({ jobs, error, retrying, onRetry, pendingJobId, onUnassign }: AssignedJobsCardProps) {
    ...
    return (
      <CardContent className="py-5">
        <div className="rounded-lg border border-dashed px-4 py-10 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">Use Assign job to give this technician work.</p>
        </div>
      </CardContent>
    )
  }
  ...
  return (
    <ul aria-label="Assigned jobs" className="divide-y">
      {sortBySchedule(jobs).map((job) => (
        <JobItem key={job.id} job={job} pendingJobId={pendingJobId} onUnassign={onUnassign} />
      ))}
    </ul>
  )
}
